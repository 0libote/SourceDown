...
`main.js` is generated by `npm run build` and ignored by git. To copy release assets into a local vault, set `SOURCE_DOWN_DEPLOY_DIR`, for example `SOURCE_DOWN_DEPLOY_DIR="$HOME/.obsidian/plugins/sourcedown" npm run deploy`.
